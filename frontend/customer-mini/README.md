...
- **首页**（`pages/index/`）：会员信息、统计面板、今日通行资格、活动 banner、快捷入口
- **预约**（`pages/tickets/`）：三步预约流程（选服务 → 填信息 → 确认），支持门票/洗护/寄养
- **订单**（`pages/orders/`）：订单列表（全部/待支付/待使用/已完成），支付重试，**入园凭证跳转**
- **入园凭证**（`pages/pass-qr/`）：动态二维码，30 秒自动刷新，一次性令牌防多人使用，非本人操作举报
- **卡种**（`pages/cards/`）：会员卡列表与状态展示
- **宠物**（`pages/pets/`）：宠物档案列表
- **我的**（`pages/profile/`）：会员资料与统计
